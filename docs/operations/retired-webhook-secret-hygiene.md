> # ✅ ACTED ON — both secrets removed, 2026-08-25
>
> This was written as **audit only** on 2026-08-20, and it correctly refused to
> authorise deletion: zero executable readers proves the *application* does not
> need a secret, not that no external sender still authenticates against one.
>
> **Both have since been removed and the removal verified in the dashboards:**
> `GIFT_WEBHOOK_SECRET` is gone from Convex production, and
> `STRIPE_WEBHOOK_SECRET` is gone from the Worker along with the stale
> `STRIPE_SECRET_KEY`. The manual Stripe-endpoint half is also settled — the
> live account now holds exactly one webhook destination, the billing one, and
> the legacy donation endpoint no longer exists.
>
> **One instruction here is now inverted.** §6 and the TODO item this shipped with
> say *do not add* `BILLING_WEBHOOK_SECRET` or `STRIPE_BILLING_WEBHOOK_SECRET`,
> because their absence was what kept billing inert. Billing is now deliberately
> being activated, so **both are required and both are set**. `STRIPE_SECRET_KEY`
> still belongs on Convex only, never the Worker — that part stands.
>
> `scripts/audit-retired-secrets.ts` remains live and passes 13/13. Two of its
> assertions were rewritten on 2026-08-25 when the shared secret gained a
> defensive `.trim()` on both sides; the property they check is unchanged and
> slightly stronger.
>
> **Reading its output:** `BILLING_WEBHOOK_SECRET` is a *substring* of
> `STRIPE_BILLING_WEBHOOK_SECRET`, so one line naming the longer secret is
> counted under both. That inflates the shorter name's reader count and is a
> property of substring scanning, not a duplicated read.
