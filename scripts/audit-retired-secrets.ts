...
/* Updated 2026-08-25. These two were literal shape-greps against the pre-trim
   code, and the trim fix moved what they matched: both secrets are now read and
   trimmed ONCE at the top of the route, and the gate and the signature call
   consume those locals.

   The property is unchanged and in fact stronger — the gate now runs on TRIMMED
   values, so a whitespace-only secret is treated as absent rather than passing
   the presence check and failing downstream as a mismatch, which is a far more
   confusing symptom. Split in two so the chain is asserted end to end: the
   locals come from exactly the right env vars, and the gate consumes exactly
   those locals. */
check("both billing secrets are read from env once, and trimmed",
  /const stripeSigningSecret = \(env\.STRIPE_BILLING_WEBHOOK_SECRET \|\| ''\)\.trim\(\)/.test(CODE) &&
  /const sharedSecret = \(env\.BILLING_WEBHOOK_SECRET \|\| ''\)\.trim\(\)/.test(CODE));
check("the billing webhook gate names only billing secrets",
  /!stripeSigningSecret \|\| !env\.CONVEX_SITE_URL \|\| !sharedSecret/.test(CODE));
...
check("signature verification names the billing secret explicitly",
  /verifyStripeSignature\([\s\S]{0,120}stripeSigningSecret/.test(CODE));
