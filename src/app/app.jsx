...
// The Declare "brain" (system prompt + generateContent + isCompleteResult) lives
// in one place now. / passes { temperature: 1.0 } at its call sites below to keep
// its long-standing value; /today passes 0.9. See src/app/declare/declare-api.js.
import { generateContent, isCompleteResult } from './declare/declare-api.js';
...
function App() {
  ...
  const runGen = async (struggle) => {
    ...
    try {
      let ai = await generateContent(struggle, translation, usedRefs, { temperature: 1.0 });
      if (!isCompleteResult(ai)) {
        console.warn('[gen] incomplete result — retrying once:', ai);
        ai = await generateContent(struggle, translation, usedRefs, { temperature: 1.0 });
      }
      if (!isCompleteResult(ai)) {
