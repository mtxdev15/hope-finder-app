/* =========================================================================
   ...
   Hybrid: tries window.claude.complete (live AI). If it is unavailable, slow,
...
(function(){
  ...
  function generateDay(o){
    return new Promise(function(resolve){
      if(!(window.claude && window.claude.complete)){ resolve(null); return; }
      var done=false;
      var to=setTimeout(function(){ if(!done){ done=true; resolve(null); } }, o.timeout||13000);
      try{
        window.claude.complete({ messages:[{role:'user', content:buildPrompt(o)}] })
          .then(function(text){ if(done) return; done=true; clearTimeout(to); resolve(parseJSON(text)); })
