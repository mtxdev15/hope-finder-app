...
3. **Cinematic opening — "the weight, gathered"** (the emotional hook; spec below). Scroll-pinned narrative: scattered heavy thoughts → gathered into the phone → answered by the Word.
4. **"See how it works" — the scroll-story** (the functional centerpiece; spec below). Flows directly out of the cinematic opening.
5. **Why Declare** — the differentiation: not just a verse — a verse, a reflection, declarations to speak aloud, and a prayer. (Quietly sets you apart from verse-only tools.)
6. **For the man carrying it alone** — who it's for; warm, reverent, Romans 12:2.
7. **iOS coming soon** — the App Store "coming soon" slot (§8).
8. **Footer** (§7).

**The cinematic opening spec — "the weight, gathered" (the emotional hook):**
Technique reference: **scroll-pinned sections with scroll-tied progress**, like gte.xyz and the Goldsand product scroll (the section stays pinned while its content animates as scroll position advances — a phone that tilts flat and rises, elements moving in 3D). Use **our** brand, never theirs: cinematic near-black forest field (§3), gold core-glow, Cormorant Garamond, reverent and warm — not crypto/flashy. The metaphor: **scattered lies → gathered by Declare → answered by the Word → identity renewed (Romans 12:2).**
- **Beat 1 — Arrival, the weight.** The section pins on the dark forest field, which fills with **fragments of heavy thoughts** as floating text (Cormorant/DM Sans) drawn from our real struggles — drifting slowly, scattered, slightly blurred, heavy (a restless 3am mind). Locked fragment lines: *"I'm not enough." · "I keep failing." · "Nobody really sees me." · "I can't shake this anxiety." · "I'm so tired of pretending." · "I can't keep it together." · "What if God's done with me?" · "Maybe this is just who I am now." · "I should have it together by now."* (Use these verbatim; they are the emotional core. They are grounded in how men actually search and describe their pain — worthlessness, failure/burnout, masking, isolation, provider pressure, and spiritual distance — and are deliberately kept in the heavy-but-survivable range. **Do NOT add any suicidal-ideation or self-harm language to these fragments;** crisis is handled only via the dedicated "In crisis? Find help now →" link, never as ambient marketing copy.)
- **Beat 2 — The turn, Declare receives.** As scroll advances, a **phone tilts from upright to flat in 3D perspective** (the Goldsand lay-down motion); the floating thoughts are **drawn down and fall into the phone.** The chaos gathers into the device and quiets.
- **Beat 3 — The lift, transformation.** The phone **rises back upright**; its screen resolves the noise into **one clear Scripture verse** (large Cormorant + gold glow), then a **declaration**, then a **prayer** — styled identically to the real Results screen (§6, screen 4). The lies are answered by the Word.
- **Hand-off:** flows directly into the four functional beats below (Name it → Receive the Word → Speak it aloud → Pray). The opening is the *feeling*; the beats are the *mechanics*. One connected flow, not two competing animations.
- **Behavior/technical:** scroll-pinned + scroll-tied (CSS `position: sticky` / scroll-driven animation / IntersectionObserver — never janky scroll listeners); **auto-plays** with scroll, no interaction required; transform/opacity only with CSS 3D transforms for the tilt; modest fragment count so it stays smooth on mobile Safari. **`prefers-reduced-motion`:** clean static fallback — three still frames (scattered thoughts → phone gathering them → verse/declaration/prayer) with the copy, no motion or pinning.
- **Responsive:** desktop = large centered phone with thoughts filling the wide field around it; mobile = phone as focus, thoughts in the space around it. Both fully smooth.
- **Tone guardrails:** reverent, never gimmicky, spooky, or game-like. Thoughts feel weighty and human; the resolution feels like light breaking in and relief. A sanctuary, not a product demo.
- **SEO:** the thought-fragments and the resolved verse/declaration/prayer are real crawlable DOM text, not images.

**The scroll-story spec (functional beats — flows out of the cinematic opening):**
